...
El estudio adopta un enfoque cuantitativo basado en análisis estadístico descriptivo y técnicas de agrupamiento aplicadas a datos secundarios provenientes del Instituto Nacional de Estadística (INE).

El procesamiento y análisis de la información se realizó en el entorno de programación R, utilizando RMarkdown para la generación reproducible de resultados en formato HTML y PDF.

Las etapas principales del análisis incluyeron:

* Limpieza y preparación de datos  
* Depuración de registros y estandarización de variables  
* Conversión de variables categóricas a factores con etiquetas oficiales  
* Tratamiento de valores faltantes y exclusión de categorías "Ignorado" cuando fue pertinente  
* Análisis descriptivo  
* Cálculo de proporciones anuales de maternidad adolescente  
* Análisis por nivel educativo  
* Análisis por grupo étnico  
* Análisis del lugar de ocurrencia del nacimiento  
* Análisis territorial por departamento  
* Identificación de valores extremos  
* Construcción de indicadores agregados por departamento  
* Aplicación de algoritmo K-means para identificación de patrones territoriales  
* Validación del agrupamiento mediante índice de silhouette
...
| Deprem       | Departamento de residencia de la madre       | Categórica |
...
ggplot(prop_educ %>% filter(Escolam != "Ignorado"),
       aes(x = reorder(Escolam, -prop_adolescente),
           y = prop_adolescente)) +
  geom_bar(stat = "identity", fill = "#2ca02c") +
  labs(title = "Maternidad Adolescente por Nivel Educativo",
       x = "Nivel Educativo",
       y = "Proporción") +
...
ggplot(prop_etnia,
       aes(x = reorder(Pueblopm, -prop_adolescente),
           y = prop_adolescente)) +
  geom_bar(stat = "identity", fill = "#9467bd") +
  labs(title = "Maternidad Adolescente por Grupo Étnico",
       x = "Grupo Étnico",
       y = "Proporción") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot(prop_sitio_educ %>% filter(Escolam != "Ignorado"),
       aes(x = Sitioocu, y = prop, fill = Escolam)) +
  geom_bar(stat = "identity") +
  labs(title = "Nivel Educativo por Lugar de Ocurrencia",
...
top_deptos <- prop_depto %>%
  filter(Deprem != "Ignorado") %>%
  slice_max(prop_adolescente, n = 10)

ggplot(top_deptos,
       aes(x = reorder(Deprem, prop_adolescente),
           y = prop_adolescente)) +
  geom_bar(stat = "identity", fill = "#d62728") +
  coord_flip() +
  labs(title = "Departamentos con Mayor Prevalencia",
       x = "Departamento",
       y = "Proporción")

bottom_deptos <- prop_depto %>%
  filter(Deprem != "Ignorado") %>%
  slice_min(prop_adolescente, n = 10)
ggplot(bottom_deptos,
       aes(x = reorder(Deprem, prop_adolescente),
           y = prop_adolescente)) +
  geom_bar(stat = "identity", fill = "#d62728") +
  coord_flip() +
  labs(title = "Departamentos con Menor Prevalencia de Maternidad Adolescente",
       x = "Departamento",
       y = "Proporción")
